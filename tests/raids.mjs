...
}
{
 ...
 for(const type of Object.keys(M.HEROES)){let prior;for(let level=1;level<=50;level++){const hash=signature(makeUnit(type,false,{hero:true,level}));if(prior)assert.notEqual(hash,prior,type+' '+level);prior=hash;}assert.ok(heroPortrait(type,'',12).includes('data-model-type="'+type+'"'));assert.ok(heroPortrait(type,'',12).includes('data-model-level="12"'));}
 pass('Every building level 1–15 and hero level 1–50 changes actual model geometry/colors; portraits render the matching model and level');
}
{
