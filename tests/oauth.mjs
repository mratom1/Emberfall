...
try {
 ...
 flow=await start(a,'google','signin');await callback(flow,{sub:'unlinked-person'});assert.equal((await poll(a,flow)).status,409);assert.equal(app.db.prepare("SELECT 1 FROM auth_identities WHERE subject='unlinked-person'").get(),undefined);pass('Switch-account sign-in cannot silently link a new provider to a registered village');
