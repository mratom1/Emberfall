...
export async function createApp(config={}){
 ...
 CREATE INDEX IF NOT EXISTS battle_defender ON battles(defender_id,settled);
 ...
 const raids=createRaids({db,getUser,stateOf,persist,fail,random:()=>randomInt(0,0x1000000)/0x1000000});
 ...
 function finishStoredBattle(row,b,u,s,now){
  const result=M.settleBattle(s,b);frontiers.finish(u,s,b,now);
  if(row.defender_id){const defender=getUser(row.defender_id);if(defender){const ds=stateOf(defender,now);R.settleDefender(ds,b,result,now);if(b.started)R.record(ds,{id:row.id,direction:'defense',opponent:u.name,opponentType:'player',hall:M.hallLevel(s),mode:'raid',created_at:now,result:{...result}});persist(defender,ds,now);}}
  if(b.started){const record={...result,attackerName:u.name,attackerHall:M.hallLevel(s),defenderName:b.enemy.name,defenderHall:b.enemy.hall||b.enemy.level||1,opponentType:b.enemy.kind||(b.pvp?'player':'bot'),mode:b.raid?'raid':b.realm||'campaign'};db.prepare('INSERT OR IGNORE INTO raid_log VALUES(?,?,?,?,?)').run(row.id,u.id,row.defender_id||null,JSON.stringify(record),now);}
  db.prepare('UPDATE battles SET state=?,settled=1,last_step=? WHERE id=?').run(JSON.stringify(b),now,row.id);persist(u,s,now);return result;
 }
 ...
 function snapshot(user,now=Date.now()){const s=stateOf(user,now),row=db.prepare('SELECT * FROM battles WHERE player_id=? AND settled=0 ORDER BY created_at DESC LIMIT 1').get(user.id);if(!row)raids.defend(user,s,now);R.ensure(s,now);s.raids.lastSeenAt=now;const battle=row?advanceBattle(row,user,s,now):null;persist(user,s,now);return {state:s,battle,battleId:row?.id||null,user:safeUser(user),serverTime:now};}
 function command(user,a,now){
  ...
  if(a.type==='battle-start'&&a.mode&& !['campaign','match'].includes(a.mode)){
  ...
  if(a.type==='battle-start'){
   ...
   let b,defenderId=null;
   if(a.mode==='match'||a.defender){const matched=raids.start(user,s,a,now);b=matched.battle;defenderId=matched.defenderId;}
   else{let index=Number(a.index||0);if(!Number.isInteger(index)||index<0||index>10000)fail('Invalid stronghold.');const max=Object.keys(s.cleared).length?Math.max(...Object.keys(s.cleared).map(Number))+1:0;if(index>max)fail('Win the preceding stronghold first.');b=M.createBattle(s,index);}
   b.id=randomUUID();persist(user,s,now);
  ...
  if(a.type.startsWith('battle-')){
   ...
   R.startAttack(s,b,now);if(b.ended&&!b.settled)finishStoredBattle(row,b,user,s,now);db.prepare('UPDATE battles SET state=?,last_step=?,settled=? WHERE id=?').run(JSON.stringify(b),now,b.settled?1:0,row.id);persist(user,s,now);return {state:s,battle:b,battleId:b.id,result:result||b.result,serverTime:now};
  }
  if(active)fail('Return home before changing your village.');if(a.type==='shield-buy'&&raids.busy(user.id))fail('Your village is under attack. Buy a Shield when the battle ends.');result=frontiers.handle(user,s,a,now)||M.applyAction(s,a,now,()=>randomInt(0,0x1000000)/0x1000000);if(result.error)fail(result.error);persist(user,s,now);return {state:s,result,serverTime:now};
 }
 ...
 const defenseTick=setInterval(()=>{const now=Date.now();for(const u of db.prepare("SELECT * FROM players WHERE COALESCE(json_extract(state,'$.raids.nextDefenseAt'),?)<=? AND COALESCE(json_extract(state,'$.shieldUntil'),0)<=? ORDER BY updated_at ASC LIMIT 5").all(now+R.DEFENSE_INTERVAL,now,now)){try{tx(()=>{const user=getUser(u.id),state=stateOf(user,now);raids.defend(user,state,now);});}catch(e){console.error('Defense update failed:',e.message);}}},60000);defenseTick.unref();
 ...
 return {server,db,settings,backups,listen:()=>new Promise(resolve=>server.listen(port,config.host||process.env.HOST||'0.0.0.0',resolve)),close:()=>new Promise(resolve=>{clearInterval(cleanup);clearInterval(battleTick);clearInterval(defenseTick);server.close(async()=>{await backups?.close();db.close();resolve();});})};
}
